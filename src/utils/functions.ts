...
export const formateToUpperCase = (data: string): string =>
  `${data.slice(0, 1).toUpperCase()}${data.slice(1)}`;

export const formatePressure = (data: number): string =>
  `${Math.round(data * 0.75006375541921)} мм рт.ст.`;

export const formateVisibility = (data: number): string => `${data / 1000} км.`;

export const formateTime24 = (data: number): string =>
  `${data < 10 ? "0" + data : data}`;

export const formateDayWeek = (
  dayArr: string[],
  monthArr: string[]
): string => {
  const date = new Date();
